...
source("Malnutrition statistical analysis/Malnutrition-study/Malnutrition_data cleaning_snack.R")
...
mna_sv_visits_95ci<-tidy(boot(data = nutri_hcru_cov %>% 
                                filter(fu>0,
                                       !is.na(mna_screening1_cat),
                                       !is.infinite(sv_num),
                                       !is.na(sv_num)), 
...
mna_pv_visits_95ci<-tidy(boot(data = nutri_hcru_cov %>% 
                                filter(pv_fu>0,
                                       !is.na(mna_screening1_cat),
                                       !is.infinite(pv_num),
                                       !is.na(pv_num)), 
...
# 4.2. Average cost by nutritional status defined by MNA ----
set.seed(2025)
mna_ov_costs_95ci<-tidy(boot(data = nutri_hcru_cov %>% 
                               filter(fu>0,
                                      !is.na(mna_screening1_cat),
                                      !is.infinite(ov_cost),
                                      !is.na(ov_cost)), 
...
mna_sv_costs_95ci<-tidy(boot(data = nutri_hcru_cov %>% 
                               filter(fu>0,
                                      !is.na(mna_screening1_cat),
                                      !is.infinite(sv_cost),
                                      !is.na(sv_cost)), 
...
mna_pv_costs_95ci<-tidy(boot(data = nutri_hcru_cov %>% 
                               filter(pv_fu>0,
                                      !is.na(mna_screening1_cat),
                                      !is.infinite(pv_cost),
                                      !is.na(pv_cost)), 
...
for (i in c("ov","sv","pv")) {
  ...
  assign(
    ...
    ggplot(temp_num,aes(x=cat,y=est)) +
      ...
      labs(x="",y="Annual number of visits and 95% CI",title="") +
...
pdf("Malnutrition statistical analysis/Malnutrition-study/figure_1_combined_visits.pdf",
    width=10,height=10)
