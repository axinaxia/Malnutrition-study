...
table1::table1(~age1+education+lives1+tobacco1+mna_screening1_cat+glim_malnutr1+
                 bmi_cat_w1+mi+hf+stroke+pd+copd+ra+op+cancer+depression+dementia+
                 smell_imp+taste_imp+chew_imp|sex,
               data = nutri_hcru_cov %>% filter(fu>0) %>% 
...
annual_visits_95ci<-function(data,indices,scale,outcome,time) {
  ...
  temp_visits<-d %>% 
    group_by(.data[[scale]]) %>% 
    summarise(sum_visits=sum(.data[[outcome]]),
...
summary(pool(mna_mort),conf.int = TRUE,conf.level = 0.95) %>%
  mutate(model=1,
         scale="mna") %>% 
