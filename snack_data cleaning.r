...
# load data
# VAL database (primary care), only available for 2001-2012
val_2001_2012<-read_sav("npr and val/ORV_val_SNAC-K_C1_C2.sav") %>% 
  as.data.frame()

val_2001_2012<-val_2001_2012 %>% 
...
# malnutrition, attrition, and some covariates data
...
med_hist_w2<-read_sav("diagnosis and anamnesis wave 1-3/wave 2/SNAC-K anamesis follow-up wave 2 clean.sav") %>% 
  as.data.frame()

med_hist_w3<-read_sav("diagnosis and anamnesis wave 1-3/wave 3/SNAC-K anamnesis follow-up wave 3 clean.sav") %>% 
  as.data.frame()
...
phy_diag_w2<-read_sav("diagnosis and anamnesis wave 1-3/wave 2/SNAC-K 14 DIAGNOS follow-up wave 2 clean.sav") %>% 
  as.data.frame()

phy_diag_w3<-read_sav("diagnosis and anamnesis wave 1-3/wave 3/SNAC-K 14 ICD-diagnosis follow-up wave 3 clean.sav") %>% 
  as.data.frame()
...
nurse_int_w2<-read_sav("Nurse wave 1-3/wave2/SNAC-K nurse wave 2 follow-up .sav") %>% 
  as.data.frame()

nurse_int_w3<-read_sav("Nurse wave 1-3/wave3/SNAC-K nurse wave 3 follow-up.sav") %>% 
  as.data.frame()


# death information collected by SNAC-K coordinator
...
# 1. Formal care data cleaning, restrict to between 2001-2019 ----
#**************************************************************************************
# Define follow-up period
# 1. Follow until: Study withdrawal, death, or end of observational period
# 2. For analysis of primary care use: end of observational period: 2011-12-31
# 3. For other HCRU: end of 2019-12-31
...
attri<-nutri_attr_cov %>% 
  ...
  mutate(fu_date_max=as.Date(pmax(date1,date2,date3,date4,
         ...
         pv_fu_date_max=as.Date(pmin(pmax(date1,date2,date3,date4,na.rm = T),as.Date("2011-12-31"),na.rm = T)),
         pv_death=ifelse((status_wave2==11|status_wave3==11|
                           status_wave4==11)&death_date<=pv_fu_date_max,1,0),
         pv_death=ifelse(is.na(pv_death),0,pv_death),
         pv_death_date=as.Date(ifelse(pv_death==1&ymd(death_date)<=as.Date("2011-12-31"),ymd(death_date),NA)),
         pv_fu_date_max=as.Date(ifelse(pv_death==1,pv_death_date,pv_fu_date_max)),
...
# Avoid extreme estimates by restricting to fu-fu_year_max>=0.25
# Merge records for fu-fu_year_max<0.25 into previous follow-up year
...
mvo_npr<-mvo_npr %>% 
  ...
  pivot_longer(
    cols = starts_with(c("mvo_", "cost_")),  # or just everything except ID
...
# 1.2. Primary care ----
# 1.2.1. Remove records that are in NPR ----
pv_hcru<-val_2001_2012 %>% 
  rename(lopnr=Löpnr) %>% 
  mutate(hcru_year=as.numeric(substr(bdat,1,4)),
         pv_date=ymd(bdat)) %>% 
  left_join(npr_before2017 %>% 
              select(Löpnr,INDATUMA) %>% 
              rename(lopnr=Löpnr) %>% 
              filter(as.numeric(substr(INDATUMA,1,4))<=2013),
            relationship = "many-to-many") %>% 
  mutate(INDATUMA=ymd(INDATUMA),
         dup_record=ifelse(pv_date==INDATUMA,1,0)) %>% 
  filter(dup_record==0) %>% 
  select(-c(INDATUMA)) %>% 
  ...
  mutate(fu=as.numeric((pv_fu_date_max-date1)/365.25),
         fu_year=as.integer((pv_date-date1)/365.25))


# set follow-up years and corresponding dates for primary care
pv_fu_years<-pv_hcru %>% 
  distinct(lopnr,fu) %>%
  mutate(fu_max=floor(fu)) %>%                    
  group_by(lopnr) %>%
  slice_head(n=1) %>%                              
  ungroup() %>%
  uncount(weights=fu_max + 1,.id = "fu_year") %>% 
  mutate(fu_year=fu_year-1) %>% 
  select(lopnr,fu_year)
  
# 1.2.2. Number of primary care visits per year ----
pv_num<-pv_hcru %>% 
  select(lopnr,pv_date,date1,fu_year,fu) %>%
...
# 1.2.3. Costs of primary care per year ----
pv_costs<-pv_hcru %>% 
  select(lopnr,klin,pv_date,hcru_year,date1,fu_year,fu) %>% 
  left_join(mvo_pv %>% 
              rename(klin=mvo))
...
ov_hcru<-ov_after2016 %>% 
  ...
  mutate(fu=as.numeric((fu_date_max-date1)/365.25),
         fu_year=as.integer((INDATUMA-date1)/365.25))


# set follow-up years and corresponding dates for outpatient care
ov_fu_years<-ov_hcru %>% 
  distinct(lopnr,fu) %>%
  mutate(fu_max=floor(fu)) %>%                    
  group_by(lopnr) %>%
...
# 1.3.1. Number of outpatient visits per year ----
ov_num<-ov_hcru %>% 
  select(lopnr,INDATUMA,date1,fu_year,fu) %>%
  group_by(lopnr,fu_year,fu) %>% 
  summarise(ov_num=n()) %>% 
  ungroup %>% 
...
# 1.3.2. Costs of outpatient care per year ----
ov_costs<-ov_hcru %>% 
  select(lopnr,MVO,INDATUMA,date1,hcru_year,fu,fu_year) %>% 
  left_join(mvo_npr %>% 
              rename(MVO=mvo) %>% 
              mutate(MVO=as.numeric(MVO)))
...
# set follow-up years and corresponding dates for inpatient care
sv_fu_years<-sv_hcru %>% 
  distinct(lopnr,fu) %>%
  mutate(fu_max=floor(fu)) %>%                    
  group_by(lopnr) %>%
...
# 1.4.1. Inpatient care visits and length of stay ----
sv_num<-sv_hcru %>% 
  select(lopnr,INDATUMA,date1,fu_year,fu) %>%
  group_by(lopnr,fu_year,fu) %>% 
  summarise(sv_num=n()) %>% 
...
# 1.4.2. Costs of inpatient care per year ----
sv_costs<-sv_hcru %>% 
  select(lopnr,MVO,INDATUMA,date1,hcru_year,fu,fu_year) %>% 
  left_join(mvo_npr %>% 
              rename(MVO=mvo) %>% 
...
all_hcru<-ov_num %>% 
  ...
  left_join(pv_num %>% 
              left_join(pv_costs %>% 
                          rename(pv_cost=cost)) %>% 
              group_by(lopnr) %>% 
              mutate(max_fu_year=max(fu_year)) %>% 
              ungroup)

summary(all_hcru)


# 3. Covariate data cleaning ----
# 3.1. Cleaning diseases ----
#**************************************************************************************
...
# 3.2. Smell, taste chewing function, and ADL ----
#**************************************************************************************
# Meaning of answers:
...
smell_adl_w1<-nurse_int_w1 %>% 
  ...
  mutate(smell_imp=ifelse(smell %in% c(3,4),1,0),
         taste_imp=ifelse(taste %in% c(3,4),1,0),
         chew_imp=ifelse(chew!=1,1,0))
...
# 4. Combine exposure data, HCRU data, covariate data, and attrition data ----
nutri_hcru_cov<-nutri_attr_cov %>% 
  ...
  full_join(all_hcru) %>%
  left_join(attri %>% 
              select(lopnr,fu,death,pv_fu,pv_death)) %>%
  mutate_at(vars(ov_num, ov_cost, sv_num, sv_cost), ~ replace_na(., 0)) %>%
  mutate(across(c(pv_num, pv_cost),~ if_else(fu_year<=max_fu_year, replace_na(., 0), .)))
